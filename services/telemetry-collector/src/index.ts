...
const validateTask = Cadenza.createTask(
    ...
    (ctx: any, emit: any) => {
      ...
      emit('telemetry.data_validated', { valid: true, deviceId: ctx.deviceId });
    ...
    .attachSignal("telemetry.data_validated")
    .then(
      Cadenza.createTask(
        'FilterOutliers',
        (ctx: any, emit: any) => {
        ...
        .attachSignal("telemetry.outlier_detected")
        .then(
...
).doOn("global.telemetry.inserted");
