...
const coreRepo = path.join(workspaceRoot, "cadenza");
const localDependency = "file:vendor/cadenza-core-local.tgz";
const packageName = "@cadenza.io/core";
...
function getCoreTarballPath() {
  const packageJson = JSON.parse(
    readFileSync(path.join(coreRepo, "package.json"), "utf8"),
  );
  return path.join(coreRepo, `cadenza.io-core-${packageJson.version}.tgz`);
}

function ensureSourceTarballExists() {
  const tarballPath = getCoreTarballPath();
...
function packSourceTarball() {
  const packOutput = run("npm", ["pack", "--silent"], coreRepo);
  ...
  const tarballPath = path.join(coreRepo, packedName);
  if (!existsSync(tarballPath)) {
...
function sync() {
  ...
  for (const packageDir of targets) {
    ...
    copyFileSync(tarballPath, path.join(vendorDir, "cadenza-core-local.tgz"));
...
function verify(sourceTarballPath = ensureSourceTarballExists()) {
  ...
  for (const packageDir of getTargetPackageDirs()) {
    const vendorTarball = path.join(packageDir, "vendor", "cadenza-core-local.tgz");
    const lockfilePath = path.join(packageDir, "package-lock.json");

    if (!existsSync(vendorTarball)) {
      failures.push(`${packageDir}: missing vendor/cadenza-core-local.tgz`);
      continue;
    }
    ...
    const lockPackage = lockfile.packages?.["node_modules/@cadenza.io/core"];
    if (!lockPackage) {
      failures.push(`${packageDir}: package-lock missing node_modules/@cadenza.io/core`);
      continue;
    }
  ...
  if (failures.length > 0) {
    throw new Error(`Local @cadenza.io/core sync failed:\n- ${failures.join("\n- ")}`);
  }

  console.log("Local @cadenza.io/core sync is aligned.");
...
function main() {
  ...
  if (!existsSync(coreRepo) || !statSync(coreRepo).isDirectory()) {
    throw new Error(`Expected sibling repo at ${coreRepo}`);
  }
